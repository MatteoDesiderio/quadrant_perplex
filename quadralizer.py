...
def get_min_PT(assembler):
    """
    Return min P and T
    """
    fname = assembler.path + "quadrant0/" + \
            assembler.path[:-1] + "_quadrant0.dat"
    with open(fname, "r") as f:
        lines = f.readlines()

    min_info = [l for l in lines if "min p, t," in l][0].strip()
    min_PT = [c for c in min_info.rsplit(" ") if c != ""][:2]
    ...
    def assemble(self):
        ...
        for i, tab in enumerate(self.tabfiles):
            try:                         
                ...
                missing_tabfiles.append(tab)
                missing_indexes.append(i)
                parts.append(-1)

        shape = part.shape
        part_nan = np.ones(shape) * np.NaN

        for i in missing_indexes:
            parts[i] = part_nan

        _, step_T, nsteps_T = np.loadtxt(good_tab, skiprows=4, 
                                             max_rows=3)
        _, step_P, nsteps_P = np.loadtxt(good_tab, skiprows=8, 
                                             max_rows=3)
        header = np.loadtxt(good_tab, max_rows=12, dtype=str, 
                            delimiter="\n")
        col_names = np.loadtxt(good_tab, skiprows=12, max_rows=1, dtype=str, 
                               delimiter="\n")

        min_P, min_T = get_min_PT(self)
        ...
        if len(missing_tabfiles) > 0:
            print("Some tab files were not found")
            print("Replacing the following quadrants with NaNs")
            missing_tabfiles = [mt.replace("quadrant", "").rsplit("_")[-2] 
                                for mt in missing_tabfiles]
            print(missing_tabfiles)
